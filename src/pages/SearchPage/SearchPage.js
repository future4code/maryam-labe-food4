...
const SearchPage = () => {
  ...
  const  {form, onChange, getListOfRestaurants, choosedCategory, setChoosedCategory, foundRestaurants, setFoundRestaurants, serachInputOnFocus, setSerachInputOnFocus } = useContext(GlobalContext)
    // const history = useHistory();
    /* const pathParams = useParams();*/
  ...
  const renderCategorys = () => {
    ...
    //   console.log(category);
      ...
      onClick={(event) => {event.stopPropagation(); handleChooseCategory(category)}}
      >{category}</p>;
    });
  };
...
const handleChooseCategory = (category) =>{
    ...
    return (
        ...
        <SearchContainerStyle>
            ...
            required
            />
            { foundRestaurants.length ? (restaurants.length > 0 ? (restaurants) : (<p>Não encontramos :(</p>)) : (<p>Busque pelo nome dos restaurantes</p>)}
            {console.log('render')}
        </SearchContainerStyle>
        </>
    )
  ...
  const restaurants = foundRestaurants
    ...
    .filter((restaurant) =>{
        ...
        if((restaurant.category === choosedCategory) || (!choosedCategory) || (form.searchInput)){
            return true
        }else{
            return false
        }
  ...
  return (
    <>
      <SearchContainerStyle onClick={() => {console.log(foundRestaurants); setSerachInputOnFocus(false)}}>
        <Header />
        <SearchWrapper>
        <input
          type="text"
          ...
          value={form.searchInput}
          title="Insert the name of a restaurant"
          onChange={(event) => {setSerachInputOnFocus(!false); onChange(event)}}
          onClick={(event) =>{event.stopPropagation(); setSerachInputOnFocus(true)}}
